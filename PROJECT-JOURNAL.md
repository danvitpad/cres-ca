...
- **2026-04-16** — **Универсальная соцсеть подписок (Instagram/VK).** Migration `00034_universal_follows_sync.sql`: sync trigger `follows→client_master_links` (4 случая: client→master insert/delete, master→client follow-back/unfollow), индексы для mutual + поиска. Переписаны `/api/follow/route.ts` (notifications + mutual detection) и `/api/follow/list/route.ts` (mutual type + entityType resolution через join к masters/salons). Новый `/api/search/global/route.ts` — поиск по всем сущностям (клиенты/мастера/салоны) с фильтрами. Dashboard clients: 3 вкладки «Клиенты / Пользователи / Подписчики» + GlobalSearchBar с dropdown и follow-кнопками. FollowerCard обновлён: бейджи типа сущности (Клиент/Мастер/Салон). i18n расширен (entityType, filters, searchGlobal). Build ✓.
...
- **2026-04-16** — **Dashboard autonomous audit session (night).** (1) **Follow→Client auto-creation fix:** critical UX bug — followers (client_master_links) didn't appear as clients in calendar side panel. Fix: `/api/follow/crm/toggle` and `/api/follow/crm/back` now auto-create `clients` record on follow/follow-back via upsert. Migration `00039_clients_unique_profile_master.sql`: partial unique index + backfill existing followers. (2) **Realtime subscriptions:** added postgres_changes channels to `/clients`, `/services`, `/inventory`, `/finance/expenses` — data auto-refreshes on INSERT/UPDATE/DELETE. (3) **Sidebar cleanup:** removed «Online Booking» and «Team» sections from dashboard sidebar (MVP = solo master, online booking lives in client Mini App). (4) **Finance dead buttons:** wired Today filter toggle and Filters reset on `/finance` page. (5) **Calendar functional fixes** (earlier in session): settings scale applied to DayView/ThreeDayView, quick actions toggle, reset button, real booked_via analytics, client notification on appointment creation. Commits: `7df166c`, `a854c97`.
