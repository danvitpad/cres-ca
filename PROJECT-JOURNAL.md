...
> **Заметки Данила, сохранённые дословно (batch 1, из commit `fea6e28`):**

  📝 TODO: на профиле мастера добавить кнопку «Пожаловаться»
  📝 BUG: при регистрации через Mini App имя сохраняется с лишним
  пробелом
  📝 IDEA: stories можно делать платными для продвижения
  📝 FIX: сегменты клиентов не учитывают cancelled_by_client

> **Batch 2 (2026-04-14, FORGE + skool.com + 8 референс-URL)** — пришёл через чат, в git не попадал. Обработанная версия в секции **BLOCK C** ниже (C1-C4 + список URL). Если нужен оригинал дословно — Данилу нужно перевставить текст сюда, и я сохраню его «как написано».
