...
- [~] **I18N-1.** Масштабная регрессия next-intl: **50 файлов с захардкоженными RU-строками** (нарушение §4 rules). Делается пакетами по surface.
  - [x] `(dashboard)/layout.tsx` — 66 строк (sidebar nav + header + user menu + notifications panel) вынесены в `dashboard.nav.*` / `dashboard.header.*` для ru/en/uk. Commit `TBD`.
  - [ ] Остальные 49 файлов (`marketing/messages`, `calendar`, `finance/*`, `(client)/**`, `/telegram/**`).
...
- **2026-04-14** — I18N-1 batch 1: `(dashboard)/layout.tsx` полностью i18n — 66 строк (sidebar nav titles/tooltips/submenus + header CTA + notifications panel + user menu) вынесены в `dashboard.nav.*` / `dashboard.header.*` для ru/en/uk. `sidebarNav` переведён из top-level const в `useMemo(buildSidebarNav(t))`. Build ✓.
