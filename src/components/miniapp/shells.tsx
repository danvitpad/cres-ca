 ...
 *  Кнопку «Назад» НЕ рисует — по запросу 2026-05-19 используем только
 *  нативную TG BackButton (включается в telegram-provider на не-корневых
 *  маршрутах). Раньше тут была авто-кнопка, но user сказал «удали наши
 *  кастомные стрелки назад». noBack оставлен в сигнатуре для совместимости. */
export function PageHeader({
  ...
  subtitle?: string;
  right?: ReactNode;
  /** @deprecated — теперь back-arrow не рендерится никогда (см. YAML выше). */
  noBack?: boolean;
}) {
  return (
    <header
