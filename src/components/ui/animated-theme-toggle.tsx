/** --- YAML
 * name: AnimatedThemeToggle
 * description: Animated sun/moon theme toggle with SVG path animations
 * --- */
...
export const AnimatedThemeToggle = ({ className }: { className?: string }) => {
  ...
  if (!mounted) {
    return (
      <button className={cn('inline-flex items-center justify-center rounded-md p-2', className)} disabled>
    ...
    <motion.div animate={isDark ? 'checked' : 'unchecked'}>
      ...
      >
        ...
        <motion.path
          ...
          stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
          transition={{ duration }} variants={moonVariants}
