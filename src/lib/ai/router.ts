...
export async function voiceToText(params: {
  ...
  const log: AICallLog[] = [];
  // Контекст помогает модели правильно расслышать продуктовые термины:
  // CRES-CA, Mini App, Telegram, мастер, клиент, запись, календарь и т.п.
  // Без него Gemini иногда расшифровывает «CRES-CA» как «Грэс-Кэй»,
  // «Mini App» как «миние пк» и т.д. — особенно на тихом аудио или iOS mic.
  const systemPrompt =
    'Ты транскрибируешь голосовое сообщение пользователя приложения CRES-CA — ' +
    'это CRM для мастеров услуг (мастера/клиенты/записи/календарь/Telegram Mini App).\n\n' +
    'Правила:\n' +
    '1) Верни ТОЛЬКО транскрипт, без комментариев, без markdown, без кавычек.\n' +
    '2) Сохраняй язык оригинала (украинский / русский / английский).\n' +
    '3) Убирай слова-паразиты («ну», «эээ», «короче», «как бы», повторы), но ' +
    'смысл сохраняй полностью — даже мелкие детали (числа, время, имена).\n' +
    '4) Расставь знаки препинания и абзацы для читаемости.\n' +
    '5) Имена брендов: CRES-CA, Telegram, Mini App, Google, Apple — пиши как принято.\n' +
    '6) Если в речи есть конкретные числа, время, даты, цены — сохрани их точно.\n' +
    '7) Если речь обрывистая или непонятная — расшифруй максимум того что слышишь, ' +
    'не додумывай отсутствующих слов.';
