...
const SYSTEM_PROMPT = `Вы помощник CRES-CA. Мастер записывает свободным текстом информацию про партнёра — другого мастера или команду (салон, клинику), с которым у него есть договорённость о взаимной рекомендации/рекламе.
...
export async function POST(req: Request) {
  const body = await req.json().catch(() => null) as
    | { partnership_id?: string; text?: string }
    | null;
  if (!body?.partnership_id || !body?.text) {
