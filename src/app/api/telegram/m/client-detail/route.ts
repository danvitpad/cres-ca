...
async function resolveMaster(userId: string) {
...
export async function POST(request: Request) {
  const { client_id, save_note } = await request.json().catch(() => ({}));
  if (!client_id) {
  ...
  const { admin, master } = await resolveMaster(userId);
