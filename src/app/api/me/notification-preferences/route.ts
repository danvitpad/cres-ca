...
export async function GET() {
  ...
  if (!data) {
    // Row might not exist for very old profiles — return sensible default
...
export async function PUT(req: Request) {
  ...
  const cleanOffsets = Array.isArray(body.offsets_minutes)
    ? body.offsets_minutes
        ...
        .filter((n) => Number.isFinite(n) && n > 0 && n <= 60 * 24 * 30)  // max 30 days
        .slice(0, 10)
    : undefined;
  ...
  // Upsert so it works even if row somehow wasn't created by trigger
