...
const SYSTEM = `Вы — AI-помощник в финансовом разделе CRES-CA для частного мастера.
Мастер пишет вопросы или просьбы. Ваша задача — определить намерение и вернуть JSON.
...
ВАЖНО: вы не выполняешь удаление, вы только определяешь намерение и описываешь, ЧТО
...
- Если вы не уверен — ставьте "qa" и попроси уточнения.
...
${q}`;
  ...
  if (parsed.intent !== 'delete' || !parsed.filter) {
    return NextResponse.json<AiResponse>({
      intent: 'qa',
      text: parsed.text?.trim() || raw.trim() || 'Не получилось обработать запрос — попробуйте ещё раз.',
    });
  }
  ...
  if (pool.length === 0) {
    return NextResponse.json<AiResponse>({
      intent: 'qa',
      text: 'Я не нашёл записей под ваше описание за последние 30 дней. Уточни дату или сумму.',
    });
  }
