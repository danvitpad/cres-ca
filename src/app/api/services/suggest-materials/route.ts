...
const SYSTEM_PROMPT = `Вы помощник CRES-CA. Мастер описывает услугу (название, длительность, ниша),
а вы оцениваешь, сколько каждого расходника со склада мастера примерно уходит на ОДИН визит.
...
export async function POST(req: Request) {
  ...
  if (inventory.length === 0) {
    return NextResponse.json({
      suggestions: [],
      summary: 'У вас пока нет расходников на складе. Сначала добавьте материалы в Склад, тогда AI сможет посчитать дозы.',
    });
  }
