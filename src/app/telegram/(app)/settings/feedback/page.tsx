      ...
      >
        ...
        <div className="flex flex-col items-center rounded-2xl border border-emerald-200 bg-emerald-50 px-6 py-10 text-center">
          ...
          <button
            ...
            className="mt-6 rounded-[var(--brand-radius-lg)] border border-neutral-200 bg-neutral-50 px-4 py-2 text-[12px] font-semibold text-neutral-800 active:bg-neutral-100"
          >
            Написать ещё
          </button>
        </div>
    ...
    >
      ...
      <div>
        <textarea
          ...
          rows={8}
          className="w-full resize-none rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-[13px] leading-relaxed text-neutral-900 placeholder:text-neutral-400 focus:border-[var(--m-accent)]/40 focus:outline-none"
        />
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[var(--m-accent)] py-3.5 text-[14px] font-semibold text-neutral-900 active:bg-[var(--m-accent-hover)] transition-colors disabled:opacity-40"
      >
        {sending ? (
          'Отправка…'
        ) : (
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-4">
        ...
        <p className="mt-1.5 text-[12px] leading-relaxed text-neutral-700">
          Откройте нашего <a href="https://t.me/crescacom_bot" target="_blank" rel="noreferrer" className="font-semibold text-[var(--m-accent)] underline">Telegram-бота</a> и отправь команду <code className="rounded bg-white/10 px-1 text-[var(--m-accent)]">/feedback</code> или голосовое со словами «обратная связь». AI расшифрует и передаст команде CRES-CA.
        </p>
      </div>
    </motion.div>
  );
