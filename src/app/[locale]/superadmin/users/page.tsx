 ...
 * description: Searchable table of all profiles with type/subscription filters, link to detail card, and quick-delete.
 * created: 2026-04-19
 * updated: 2026-05-01
 * --- */
...
import { QuickPurgeButton } from '@/components/superadmin/quick-purge-button';
        ...
        <table className="w-full table-fixed border-collapse text-[13px]">
          <thead>
            <tr className="bg-white/[0.03] text-[11px] uppercase tracking-wider text-white/50">
              <th className="w-[22%] px-3 py-2.5 text-left font-medium">Имя</th>
              <th className="w-[9%] px-3 py-2.5 text-left font-medium">Тип</th>
              <th className="w-[21%] px-3 py-2.5 text-left font-medium">Email</th>
              <th className="w-[10%] px-3 py-2.5 text-left font-medium">Подписка</th>
              <th className="w-[9%] px-3 py-2.5 text-left font-medium">Whitelist</th>
              <th className="w-[11%] px-3 py-2.5 text-left font-medium">Город</th>
              <th className="w-[11%] px-3 py-2.5 text-left font-medium">Регистрация</th>
              <th className="w-[7%] px-3 py-2.5 text-left font-medium"></th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((r) => {
                const pill = subPill(r.subscriptionTier, r.subscriptionStatus);
                return (
                  <tr
                    key={r.id}
                  ...
                  >
                    ...
                    <td className={['truncate px-3 py-2.5', pill.cls].join(' ')}>{pill.label}</td>
                    <td className="px-3 py-2.5">
                      {r.whitelisted ? (
                    ...
                    <td className="px-3 py-2.5">
                      <QuickPurgeButton profileId={r.id} profileName={r.displayName} />
                    </td>
                  </tr>
                );
              })
