/** --- YAML
 * name: ReviewsPage
 * description: Fresha-exact reviews management — view and respond to client reviews
 * --- */
...
export default function ReviewsPage() {
  ...
  return (
    <div style={{ fontFamily: FONT, padding: '32px 40px' }}>
      <h1 style={{ fontSize: 28, fontWeight: 600, color: C.text, margin: '0 0 8px' }}>{t('reviews')}</h1>
      <p style={{ fontSize: 15, color: C.textMuted, margin: '0 0 32px', lineHeight: '22px' }}>{t('reviewsDesc')}</p>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ backgroundColor: C.emptyBg, border: `1px dashed ${C.emptyBorder}`, borderRadius: 12, padding: '64px 32px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 600, color: C.text, marginBottom: 8 }}>{t('noReviews')}</div>
        <div style={{ fontSize: 15, color: C.textMuted, maxWidth: 400, margin: '0 auto', lineHeight: '22px' }}>{t('noReviewsDesc')}</div>
      </motion.div>
    </div>
  );
}
