/** --- YAML
 * name: TimesheetsPage
 * description: Fresha-exact timesheets — track team working hours
 * --- */
...
export default function TimesheetsPage() {
  ...
  return (
    <div style={{ fontFamily: FONT, fontFeatureSettings: FONT_FEATURES, background: C.bg }}>
      <h1 style={{ fontSize: 28, fontWeight: 600, color: C.text, margin: '0 0 8px' }}>{t('timesheets')}</h1>
      <p style={{ fontSize: 15, color: C.textSecondary, margin: '0 0 32px', lineHeight: '22px' }}>{t('timesheetsDesc')}</p>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ backgroundColor: C.surface, border: `1px dashed ${C.border}`, borderRadius: 12, padding: '64px 32px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 600, color: C.text, marginBottom: 8 }}>{t('noTimesheets')}</div>
        <div style={{ fontSize: 15, color: C.textSecondary, maxWidth: 400, margin: '0 auto', lineHeight: '22px' }}>{t('noTimesheetsDesc')}</div>
      </motion.div>
    </div>
  );
}
