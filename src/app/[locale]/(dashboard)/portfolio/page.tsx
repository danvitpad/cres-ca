...
export default function PortfolioPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 p-6">
      <div>
        <h1 className="text-[28px] font-bold tracking-[-0.02em] leading-none">Портфолио</h1>
                  ...
                  {item.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {item.tags.map((t) => (
                        <span
                          key={t}
                          className="rounded-full bg-[var(--color-accent-soft)] px-2 py-0.5 text-xs text-[var(--color-accent)] dark:bg-[var(--color-accent)]/10 dark:text-[var(--color-accent-text)]"
                        >
                          #{t}
                        </span>
                      ))}
