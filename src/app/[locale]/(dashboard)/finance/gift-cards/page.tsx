...
export default function GiftCardsPage() {
  ...
  const loadCards = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select('id, code, amount, is_redeemed, created_at')
                ...
                >
                  ...
                  <td style={{ padding: '12px 20px', textAlign: 'right' }}>
                    <span style={{
                      ...
                      background: card.is_redeemed ? 'rgba(245,158,11,0.12)' : 'rgba(16,185,129,0.12)',
                      color: card.is_redeemed
                        ? (mounted && resolvedTheme === 'dark' ? '#fbbf24' : '#d97706')
                        : (mounted && resolvedTheme === 'dark' ? '#34d399' : '#059669'),
                    }}>
                      {card.is_redeemed ? t('completed') : t('booked')}
                    </span>
                  </td>
                </motion.tr>
              ))}
